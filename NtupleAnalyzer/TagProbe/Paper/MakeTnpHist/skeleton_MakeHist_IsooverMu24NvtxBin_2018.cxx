...
public:
  ...
  Bool_t IsProbe()
  {
    ...
    if( probe_.isTight && 
        ...
        MuonHLT::dRMatching_HLTObj(probe_.vecP, ntuple_, "hltL3fL1sSingleMu22L1f0L2f10QL3Filtered24Q::HLT", 0.1) )
      flag = kTRUE;
  ...
  Bool_t IsPassingProbe()
  {
    ...
    if( MuonHLT::dRMatching_HLTObj(probe_.vecP, ntuple_, "hltL3crIsoL1sSingleMu22L1f0L2f10QL3f24QL3trkIsoFiltered0p07::HLT", 0.1) )
      flag = kTRUE;
...
public:
  ...
  void Set_minVtx( int value ) { minVtx_ = value; }
  void Set_maxVtx( int value ) { maxVtx_ = value; }
  ...
  void Produce()
  {
    ...
    for(Int_t i_ev=0; i_ev<nEvent; i_ev++)
    {
      ...
      if( !( minVtx_ <= ntuple->nVertex && ntuple->nVertex < maxVtx_ ) ) continue;
...
void run(int minVtx, int maxVtx)
{
  HistProducer* histProducer = new HistProducer();
  //==== OUTPUTSAMPLE :   histProducer->Set_OutputFileName("ROOTFile_TnPHist_Example.root");
  //==== INPUTSAMPLE : histProducer->AddDataPath("/data8/DATA/MuonHLT/crab_MuonHLTNtuple_SingleMuon_Run2018D_RunAfter324315/190317_111023/0000/ntuple_1.root");
  histProducer->Set_minPt( 26 ); // -- min pT applied for eta, phi and vtx

  histProducer->Set_minVtx( minVtx );
  histProducer->Set_maxVtx( maxVtx );
