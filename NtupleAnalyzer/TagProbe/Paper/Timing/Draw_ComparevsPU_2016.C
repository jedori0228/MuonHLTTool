...
void Draw_ComparevsPU_2016(){

  gStyle->SetOptStat();
  setTDRStyle();

  TString OutDir = "output/PUStudy/2016/";
...
283353,
  ...
  double x_PUs[3] = { 25., 35., 47.5 };
  double x_PUs_err[3] = { 2., 2., 2.5 };
  ...
  for(unsigned int i_path=0; i_path<Paths.size(); i_path++){
    ...
    for(unsigned int i_PU=0; i_PU<PUs.size(); i_PU++){
      ...
      TString str_MeanTiming_Error = Precision(MeanTiming_Error,2);
      cout << MeanTiming << " -> " << str_MeanTiming << " +- " << str_MeanTiming_Error << endl;
    ...
    latex_Lumi.DrawLatex(0.72, 0.96, "Run2016");
    ...
    TCanvas *c_PU_vs_Timing = new TCanvas("c_PU_vs_Timing", "", 600, 600);
    c_PU_vs_Timing->cd();
    TGraphAsymmErrors *gr = new TGraphAsymmErrors(3, x_PUs, MeanValues, x_PUs_err, x_PUs_err, MeanValues_Error, MeanValues_Error);
    gr->Draw("ap");
    c_PU_vs_Timing->SaveAs(OutDir+"/PU_vs_Timing"+Path+".pdf");
    c_PU_vs_Timing->Close();
