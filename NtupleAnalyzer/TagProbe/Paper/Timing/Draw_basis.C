...
void Draw_basis(){
  ...
  //==== Overall
  TCanvas *c_Overall = new TCanvas("c_Overall", "", 1000, 800);
  canvas_margin(c_Overall);
  c_Overall->cd();
  c_Overall->SetLogy();
  TH1D *hist_dummy_Overall = new TH1D("hist_dummy_Overall", "", 2000, 0., 2000.);
  hist_dummy_Overall->GetYaxis()->SetRangeUser(0.000008,0.5);
  hist_dummy_Overall->GetYaxis()->SetTitle("events / 10.0 ms");
  hist_dummy_Overall->GetXaxis()->SetTitle("processing time [ms]");
  hist_dummy_Overall->Draw("axis");
  hist_axis(hist_dummy_Overall);

  TLegend *lg = new TLegend(0.34, 0.80, 0.99, 0.95);
  lg->SetBorderSize(0);
  lg->SetFillStyle(0);
  ...
  for(unsigned int i=0; i<Runs.size(); i++){
    ...
    TH1F *hist = (TH1F *)file->Get("DQMData/Run "+Run+"/HLT/Run summary/TimerService/process TIMING time_real");
  ...
  c_Overall->SaveAs(OutDir+"Overall.pdf");
  c_Overall->SaveAs(OutDir+"Overall.png");
  c_Overall->Close();
  ...
  for(unsigned int it_Path=0; it_Path<Paths.size(); it_Path++){
    ...
    hist_dummy_Path->GetYaxis()->SetRangeUser(0.000008,0.5);
    ...
    for(unsigned int i=0; i<Runs.size(); i++){
      ...
      lg2->AddEntry(hist, aliases.at(i)+" Mean: "+str_MeanTiming+" ms");
      hist->Draw("histsame");
